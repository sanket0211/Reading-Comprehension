...
for num_skips, skip_window in [(2, 1)]:
    ...
    for bi in range(8):
      print("(",end="")
      for ci in range(2):
        print(reverse_dictionary[batch[bi,ci]],end=", ")
      print(")",end=", ")
    print("]") 
    #print('    batch:', [(reverse_dictionary[bi],reverse_dictionary[ci]) for bi in batch[:,0] for ci in batch[bi,:]])
...
with tf.Session(graph=graph) as session:
  ...
  for step in range(num_steps):
    ...
    #print('    labels:', [reverse_dictionary[li] for li in batch_labels.reshape(8)])
    if step % 2000 == 0:
      if step > 0:
      ...
      average_loss = 0
      #print(session.run(valid_embeddings[15,41:50]))
